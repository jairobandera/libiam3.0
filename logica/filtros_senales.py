    ...
    if tipo == "bandpass":
        try:
            ...
            raise ErrorConfiguracionFiltro(
                "El filtro pasa-banda necesita una frecuencia inferior y otra superior."
            ) from exc
...
def _validar_configuracion(
    ...
    if not 1 <= orden <= 12:
        raise ErrorConfiguracionFiltro("El orden debe estar entre 1 y 12.")
...
def aplicar_butterworth(
    ...
    pasa-altos, y un par ``(inferior, superior)`` en el modo pasa-banda.
    """
    ...
    # Se interpolan únicamente los huecos para que el filtro pueda operar. Al
    # terminar se restituyen los NaN originales, que no deben inventar datos.
    ...
    try:
        filtrados = sosfiltfilt(sos, datos_continuos)
...
def aplicar_butterworth_pasabajos(
    ...
    El uso de secciones de segundo orden mejora la estabilidad numérica y el
    filtrado bidireccional evita desplazar temporalmente los eventos.
    """
